<template>
	<view>
		...
		<text>
			\n
			现在，请靠近您的电脑，听电脑内部的风扇是否有转动
			\n
			[笔记本请将耳朵靠近通风口倾听]
			\n
			<text class="text-xl">现在请判断：</text>
			\n
			<text style="font-weight: bold;">您的电脑内部风扇是否转动？</text>
		</text>
	...
	export default {
		...
		onLoad() {
			uni.getStorage({
				key:'choice_array',
				success: (res) => {
					console.log('读取选择分支成功 by step2-2')
			...
			uni.getStorage({
				key:'result_data',
				success: (res) => {
					console.log('读取向导结果成功 by step2-2')
		...
		methods: {
			yes(){
					const found = this.choice.find(element => element == "23" || element == "24")
					if(found == "23" || found == "24"  ){
						uni.showToast({
						title: '请勿重复选择!',
						icon:'none',
						duration: 2000
