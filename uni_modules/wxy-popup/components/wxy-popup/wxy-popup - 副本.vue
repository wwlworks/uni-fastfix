 ...
 *
 * 修改说明
 * 1、修改中心弹窗的最小宽度和最小高度，默认是100rpx，现修改为10vw和10vh，若是使用过程中存在多余时，可调整它大小（2021-1-8）
 * 2、默认宽度和高度:上下时宽度是100%，高度是auto，左右时宽度是auto，高度是100%,中心时宽度是100%，高度是auto（2021-06-15）
 * 3、关闭图标改进为菜单图标，上下左右同以前不变，中心时增加了全屏和退出全屏功能
 * 4、上下左右弹窗时可指定距离边缘的距离，应用场景更灵活了，如弹窗中放个音乐播放，这样就可以是悬浮式音乐播放了，其它可以自由发挥了
 */
...
export default {
	...
	computed: {
		...
		style() {
			...
			if (this.mode.toLowerCase() == 'left' || this.mode.toLowerCase() == 'right') {
				style = {
					...
					height: this.height ? this.addUnit(this.height) : 'auto',
		...
		centerStyle() {
			...
			style.height = this.height ? this.addUnit(this.height) : 'auto';
			style.zIndex = this.uZindex;
			style.backgroundColor = this.popColor;
	...
	methods: {
		...
		open() {
			//务必要重置heightContainer，否则计算的高度会不断累加，是因为scrollStyle的
			let style = JSON.parse(JSON.stringify(this.scrollStyle));
			this.$nextTick(() => {
